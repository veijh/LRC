...
int set_spd[4] = {19*60, 19*60, 19*60, 19*60};	
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOH, J20_Pin|J19_Pin, GPIO_PIN_SET);

  /*Configure GPIO pins : J20_Pin J19_Pin */
  GPIO_InitStruct.Pin = J20_Pin|J19_Pin;
...
void StartDefaultTask(void *argument)
{
  ...
  for(;;)
  {
	for(int i=0; i<4; i++)
	{
		...
		set_moto_current(my_hcan1->hcan, 
							pid_spd[0].pos_out, 
							pid_spd[1].pos_out,
							pid_spd[2].pos_out,
							pid_spd[3].pos_out);
